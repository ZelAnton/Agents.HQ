...
**Между репозиториями — бесплатно (на уровне ТЕКСТА).** Разные репозитории — разные рабочие
деревья, поэтому текстовых merge-конфликтов между ними нет. Это основной источник параллелизма на
старте. Но «нет текстового конфликта» ≠ «безопасно приземлять»: семантическая совместимость и
**порядок land** (потребитель — после фундамента) сохраняются — см. правило кросс-репо land в
[`IMPLEMENTATION.md §11.4`](IMPLEMENTATION.md).
